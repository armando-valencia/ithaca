...
<details>
<summary>Opening unsigned builds</summary>

Unsigned builds show Gatekeeper warnings on other Macs. To open one, right-click
`Ithaca.app`, choose **Open**, then confirm **Open** again. If macOS still blocks
the app, run this from the folder that contains it:

```sh
xattr -dr com.apple.quarantine Ithaca.app
```

Signing and notarizing the app avoids this first-launch step.
